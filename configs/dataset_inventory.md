...
- **11,676,882** total examples across active datasets
...
|---------|--------|------|-----------|-------|-------------|--------|
| b2nerd_curated | json_zipped | 388,075 | EN, ZH | 492 | pos in NER_en only; name+type elsewhere | needs_extraction |
| b2nerd_test | json_zipped | 407,036 | EN, ZH | 492 | pos in NER_en only; name+type elsewhere | needs_extraction |
| b2nerd_raw | json_zipped | 4,261,663 | EN, ZH | 492 | pos in NER_en only; name+type elsewhere | needs_extraction |
| open_ner_standardized | parquet_bio | 830,734 | 52 langs | 60 | BIO tags via ClassLabel | ready |
| open_ner_core_types | parquet_bio | 830,734 | 52 langs | 3 | BIO tags via ClassLabel | ready |
| finerweb | parquet_spans | 1,995,477 | 93 langs | 235,181 | char_spans field with start/end/label objects | ready |
...
| chinese_ner_sft | jsonl_spans | 182,293 | ZH | 72 | entity_label + start_idx/end_idx/entity_text fields | ready |
...
- **Span info**: pos field [start, end] in NER_en only (25/27 datasets); NER_zh and NER_en_other have name+type only (no position offsets). PileNER in NER_en also lacks pos.
...
- **Entity types**: 492 (AI algorithm, Abbreviation, Activity, Agreement, Anatomy, Animal, Award, Band, ...)
- **Span info**: pos field [start, end] in NER_en only (25/27 datasets); NER_zh and NER_en_other have name+type only (no position offsets). PileNER in NER_en also lacks pos.
- **Notes**: Combines B2NERD_all/ (2,102,304) and B2NERD_raw/ (2,159,359) from zip. Counts include train/dev/test JSON files only (excluding metadata). Reserve for v2 type coverage gaps.
...
- **Entity types**: 60 (LOC, ORG, PER, GPE, EVENT, DATE, MISC, PRODUCT, CORPORATION, CREATIVE_WORK, FACILITY, LAW, LANG, NORP, QUANTITY, TIME, CARDINAL, MONEY, PERCENT, ORDINAL, PER-DERIV, LOC-DERIV, ORG-DERIV, ...)
- **Span info**: BIO tags via ClassLabel — numeric codes mapped to B-TYPE/I-TYPE/O
- **Notes**: Multi-dataset collection with 66 configs across 52 languages. 60 entity types total (115 BIO labels including B-/I- prefixes). BIO format with HF ClassLabel feature. Total includes train (624,532) + dev (75,416) + test (130,786) across all configs.
...
- **Entity types**: 235,181 (abbreviation, abstract concept, academic conference, academic degree, academic discipline, animal, award, book, ...)
- **Span info**: char_spans field with start/end/label objects — native character offsets
- **Notes**: 93 languages, 235,181 unique entity types across all language files. Extreme granularity: labels include hierarchical paths (e.g. 'location / city'), context-specific values, and multilingual labels in native scripts. One parquet file per language. char_spans has list of {start, end, label} dicts.
...
- **Entity types**: 72 (Abstract, BANK, COMMENTS_ADJ, COMMENTS_N, CONT, EDU, GPE, GPE.NAM, HCCX, HPPX, LOC, TELEVISION, 疾病和诊断, 解剖部位, ...)
- **Span info**: entity_label + start_idx/end_idx/entity_text fields — character offsets. Some null values present.
- **Notes**: 12 Chinese NER subsets aggregated. 72 entity types from: bank (4), ccfbdci (4), ccks2019 (6), cluener (10), cmeee (9), dlner (7), e_commerce (4), finance_sina (8), msra (3), nlpcc_2018 (15), weibo (7), youku (3). Includes domain-specific types (medical, financial). Has null values in some entity fields. entity_label field (not entity_type).
...
| B2NERD raw | 1,419,161 | 4,261,663 | B2NERD_all/ (2,102,304) + B2NERD_raw/ (2,159,359); train/dev/test only |
| fiNERweb types | 235K+ | 235,181 | Plan was correct; initial inventory undercounted (per-language only vs cross-language unique labels) |
...
| open-ner types | ~60 | 60 | Plan was correct; 115 BIO labels → 60 unique base entity types after stripping B-/I- prefixes |
